...
class UserValidation {
  static async signUp(req, res, next) {
    try {
      ...
      if (await InputCheck.nameCheck(firstName)) {
        return Response.handleError(400, 'Enter valid first name', res);
      }
      if (await InputCheck.nameCheck(lastName)) {
        return Response.handleError(400, 'Enter valid last name', res);
      }
      if (await InputCheck.emailCheck(email)) {
        return Response.handleError(400, 'Enter valid email', res);
      }
      if (await InputCheck.phoneNoCheck(phoneNumber)) {
        return Response.handleError(400, 'Enter valid phone number', res);
      }
      if (await InputCheck.addressCheck(address)) {
        return Response.handleError(400, 'Enter valid address', res);
      }
  ...
  static async signIn(req, res, next) {
    try {
      ...
      if (!email) {
        return Response.handleError(400, 'Email is required', res);
      }
      if (!password) {
        return Response.handleError(400, 'Password is required', res);
      }
      next();
    } catch (error) {
      return Response.handleError(500, error.toString(), res);
