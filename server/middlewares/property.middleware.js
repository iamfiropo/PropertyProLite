...
class PropertyValidation {
  static async property(req, res, next) {
    try {
      ...
      if (await InputCheck.floatCheck(price)) {
        Response.handleError(400, 'Enter valid price in numeric', res);
      }
      if (await InputCheck.nameCheck(state)) {
        Response.handleError(400, 'Enter valid state', res);
      }
      if (await InputCheck.nameCheck(city)) {
        Response.handleError(400, 'Enter valid city', res);
      }
