...
class UserValidation {
  static async signUp(req, res, next) {
    try {
      ...
      if (await InputCheck.nameCheck(firstName)) Response.handleError(400, 'Enter valid first name', res);
      if (await InputCheck.nameCheck(lastName)) Response.handleError(400, 'Enter valid last name', res);
      if (await InputCheck.emailCheck(email)) Response.handleError(400, 'Enter valid email', res);
      if (await InputCheck.phoneNoCheck(phoneNumber)) Response.handleError(400, 'Enter valid phone number', res);
  ...
  static async signIn(req, res, next) {
    try {
      ...
      if (await InputCheck.emailCheck(email)) Response.handleError(400, 'Enter valid email', res);
      if (await InputCheck.passwordCheck(password)) Response.handleError(400, 'Enter valid password', res);
      next();
    } catch (error) {
      Response.handleError(500, error.toString(), res);
    }
