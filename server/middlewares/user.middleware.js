...
class UserValidation {
  static async signUp(req, res, next) {
    try {
      ...
      if (await InputCheck.nameCheck(first_name)) return Response.handleError(400, 'Enter valid first name', res);
      if (await InputCheck.nameCheck(last_name)) return Response.handleError(400, 'Enter valid last name', res);
  ...
  static async signIn(req, res, next) {
    try {
      ...
      if (await InputCheck.emailCheck(email)) return Response.handleError(400, 'Enter valid email', res);
      if (await InputCheck.passwordCheck(password)) return Response.handleError(400, 'Enter valid password', res);
      next();
    } catch (error) {
      return Response.handleError(500, error.toString(), res);
    }
